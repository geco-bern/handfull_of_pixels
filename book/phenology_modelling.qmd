...
With a basic model parameterized you can explore if and how the relationship between observed and predicted values scale across a larger landscape. As example we will use a region around Boston in the eastern United States, defined by the coordinates -72 to -70 East, and 42 to 44 North.
I will use DAYMET raster data to scale the results spatially (relying on the same driver data as used during parameterization). First I download both minimum and maximum temperature data and average them to a mean daily value (as previously used), using the `appeears` R package [@hufkens2023].

To do so, first create a NASA Earth Data user and setup the appeears package with that user (see more detailed informatio in the [appeears package documentation](https://bluegreen-labs.github.io/appeears/articles/appeears_vignette.html).
...
rs_set_key()
# if the package has no access to your system password manager, potentially use options(keyring_backend="file") before attempting to set a key

# To see whether the setup has worked, you can try to get the metadata of the product DAYMET v4:
rs_layers("DAYMET.004")
```

Then I proceed to download the data. Note that this might take a while since the request is first queued and then prepared before it is available for download. The progress can be followed under: https://appeears.earthdatacloud.nasa.gov/explore.
...
In case there are issues with creating a user account or the download you can use:
```{r eval = FALSE, warning = FALSE}
r1 <- terra::rast("https://github.com/geco-bern/handfull_of_pixels/data/DAYMET.004_2012/DAYMET.004_1km_aid0001.nc?raw=TRUE")
# Assign the correct CRS, e.g. WGS84 (EPSG:4326)
terra::crs(r1) <- "epsg:4326"
```
...
mean_layer <- terra::mean(r1["tmax"], 
                          r1["tmin"])
varnames(mean_layer) <- "tmean"                             # fix the variable naming
names(mean_layer) <- gsub("tmax","tmean",names(mean_layer)) # fix the variable naming
```

This results in a raster containing a single variable 'tmean' for each day:
```{r}
#| code-fold: true
#| label: fig-tmean-daily
#| fig-cap: "Daily mean temperature for the first 16 days of 2012 for the greater Boston area."
#| fig-align: "center"
#| out-width: "100%"
